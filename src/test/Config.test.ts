...
import { extractBuildConfigs, extractRoutes, extractSecurity, printConfigSummary, printSecuritySummary, printContractReport, createConfig } from '../Config';
...
describe('createConfig', () => {
  it('returns the same object when at least one app is present', () => {
    ...
    const out = createConfig(cfg);
    expect(out).toBe(cfg);
  });

  it('throws if no apps configured', () => {
    const cfg: TaujsConfig = { apps: [] };
    expect(() => createConfig(cfg as any)).toThrow('At least one app must be configured');
  });
});
